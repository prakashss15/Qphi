...
// ── Build the correct assignees aggregation based on DB type ──
  ...
  if (db.isSQLite) {
    return `
      SELECT
        ...
        COALESCE(
          json_group_array(
            json_object('id', u.id, 'name', u.name, 'email', u.email, 'avatar_color', u.avatar_color)
...
// Helper to parse assignees (SQLite returns JSON string, PG returns object)
...
router.get('/:id', async (req, res) => {
  ...
  try {
    const BASE_QUERY = getBaseQuery();
    const result = await db.query(`${BASE_QUERY} WHERE t.id = $1 GROUP BY t.id`, [id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Task not found' });
    res.json(parseTasks(result.rows)[0]);
  } catch (err) {
    console.error(err);
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (assignee_ids && assignee_ids.length > 0) {
      for (const uid of assignee_ids) {
        await db.query(
          ...
          db.query('INSERT INTO task_assignees (task_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [task.id, uid])
        );
      }
    }
    ...
    const BASE_QUERY = getBaseQuery();
    const fullTask = await db.query(`${BASE_QUERY} WHERE t.id = $1 GROUP BY t.id`, [task.id]);
    res.status(201).json(parseTasks(fullTask.rows)[0]);
  } catch (err) {
    console.error(err);
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const BASE_QUERY = getBaseQuery();
    const fullTask = await db.query(`${BASE_QUERY} WHERE t.id = $1 GROUP BY t.id`, [id]);
    if (fullTask.rows.length === 0) return res.status(404).json({ error: 'Task not found' });
    res.json(parseTasks(fullTask.rows)[0]);
  } catch (err) {
    console.error(err);
...
// PATCH update task status only (drag-drop)
...
router.delete('/:id/assignees/:user_id', async (req, res) => {
  ...
  try {
    await db.query('DELETE FROM task_assignees WHERE task_id = $1 AND user_id = $2', [id, user_id]);
